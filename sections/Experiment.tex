...
This is necessary for smaller dataset to ensure convergence before the algorithm finishes, but for large enough datasets this parameter can have a value of one, as is the case for \cite{online} where their corpus includes 3.3M documents where our dataset is ~32000 documents.
\texttt{iterations} which adjusts the maximum number of iterations of the E-step (from \cite{}, Algorithm 2) is allowed without all documents having converged. 
Setting this to a high value ensures convergence, but also increases training time. 
In practice, iterations should be set to the lowest possible values where nearly (> 99\%) all documents have converged by the end of the training.
...
This theoretical optimal value is however based on the corpus, so will need to be tested to find some setting of K that is close to the optimal.
...
To evaluate the \gls{lda} models, we measured perplexity \todo{explain perlexity}.
During the first grid-search, $10$ and $50$ were clearly the best values for K.
...
\todo[inline]{what do we do about one of the alpha values in the first gridsearch being set wrong (0.0001 instead of 0.001)?}

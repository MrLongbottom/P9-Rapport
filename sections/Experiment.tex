...
\Gls{map} is calculated as follows\footnote{Equations from \url{https://towardsdatascience.com/breaking-down-mean-average-precision-map-ae462f623a52} and \url{https://en.wikipedia.org/wiki/Evaluation_measures_(information_retrieval)\#Mean_average_precision}}:
...
\noindent Where $TP(n)$ is the number of documents correctly labeled as relevant for the query, in the $n$ top ranked documents, and $FP(n)$ is the number of documents incorrectly labeled as relevant for the query, in the $n$ top ranked documents.

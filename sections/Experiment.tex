...
For topic queries, all documents with values higher than the mean of the topic's vector in the document-topic matrix $\theta_t$ are considered correct retrievals.
...
However since document queries only contain one ground truth positive, $p @ n$ doesn't make much sense to show. 
...
Specifically, we hypothesize that the \gls{lda} is able to improve the topic \gls{ir} when combined with other baselines and that \gls{pr} algorithms operating on a graph built based on topic similarity is able to improve results when combined with other baselines.

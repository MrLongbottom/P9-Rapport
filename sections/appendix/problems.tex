...
However, because all word probabilities $P(w|d)$ are multiplied together, having a value of 0 just one word will make the whole query have a value of 0.
...
Queries that contain one 0-value words are also evaluated the same as queries containing more 0-value words.
...
This value is so low that the query will always score higher than queries containing no 0-value words, but it makes it possible to sort queries containing one or more 0-value words.

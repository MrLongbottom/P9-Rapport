...
In the generative process of lda, a document is assumed to have a certain underlying topic-distribution and a word is assumed to have a certain underlying word-distribution. Words are generated in the document by choosing a topic from the document based on the topic-distribution of the document and then choosing a word from the topic based on the word-distribution of that topic.
...
This would be done using the same tables that were used to create the words $\theta$ and $\varphi$, but by accessing columns rather than rows, which also meant that the array of numbers we received had to be converted into distributions, as only the rows of these matrices were guareentied to sum to 1.
...
Our hope was that each word would only be a significant part of a few topics, and that each topic was only a significant part of a few document. If the topic-distribution only contained a few high values, one or more of which would be among the topics belonging to the original document (which is a stand-in for a relevant document in this case). And the original document would then hopefully be a significant part of these topics, leading us back to the document.
...
While this resulted in more decisive distributions, the end results was very bad, often ranking the original document very low.
Futhermore significantly under half the words would even have a single matching topic with the original document, despite usually having more topics in their distribution that the document's distribution.
